classdef sortomatorect < imrect & dynamicprops
    %   SORTOMATORECT An imrect subclass with a label option
    %
    %   Syntax
    %   ------
    %   hRect = sortomatorect
    %   hRect = sortomatorect(hAxes)
    %   hRect = sortomatorect(hAxes, position)
    %   hRect = sortomatorect(...,param1, val1, ...)
    %
    %   Description
    %   -----------
    %   hRect = sortomatorect interactively creates a rectangle ROI on the
    %   current axes.
    %   
    %   hRect = sortomatorect(hAxes) interactively creates a rectangle ROI on the
    %   axes specified by the handle hAxes.
    %
    %   hRect = sortomatorect(hAxes, position) creates a draggable, resizable
    %   rectangle ROI on the axes specified by the handle hAxes at the
    ...
    %   rect bounding box. The last two elements specify the width and
    %   height of the bounding box.
    %
    %   hRect = sortomatorect(...,param1, val1, ...) specifies additional
    %   parameter value pairs that control the rect. See imrect for
    %   valid Parmater and Value arguments.
    %
    %   Because sortomatorect is a subclass for imrect, object creation
    %   syntax is identical to imrect. Type help imrect for more
    %   information.
    %
    ...
    methods
        ...
        function obj = sortomatorect(varargin)
        ...
        function updateLabels(obj,~)
            ...
            rgnURCorner = [objPos(1) + objPos(3)*1.1, objPos(2) + objPos(4)];
        ...
        function delete(obj)
            % delete sortomatorect subclass destructor function
            %
            %
        ...
        function deleteSortomatoRgn(obj)
            % delete the rectangle region from the Sortomato graph
            %
            %
        ...
        function setLabel(obj)
            ...
            if ~isempty(obj.Name)
                % Get the rectangle's center. For a rectangle imroi, the
